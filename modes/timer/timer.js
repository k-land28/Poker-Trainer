...
mainContent.innerHTML = `
  <h1 id="timer-title"> - Level <span id="timer-level"></span></h1>
  ...
  <div class="timer-controls">
    <button id="timer-pause-btn">
      <svg class="icon"><use href="../data/icons-sprite.svg#icon-play" /></svg> Start
    </button>
    <button id="timer-skip-btn">
      <svg class="icon"><use href="../data/icons-sprite.svg#icon-double-play" /></svg> Skip Level
    </button>
    <button id="timer-seatopen-btn">
      <svg class="icon"><use href="../data/icons-sprite.svg#icon-user-minus" /></svg> シートオープン
    </button>
    <button id="timer-rebuy-btn">
      <svg class="icon"><use href="../data/icons-sprite.svg#icon-user-plus" /></svg> リエントリー
    </button> 
    <button id="timer-reset-btn">
      <svg class="icon"><use href="../data/icons-sprite.svg#icon-refresh" /></svg> Reset
    </button>
    <button id="timer-config-btn">
      <svg class="icon"><use href="../data/icons-sprite.svg#icon-settings" /></svg> 設定
    </button>
  </div>
`;
  ...
  if (!state.config) {
    fetch('././data/timer/timer_config.json')
      ...
      .then(data => {
        ...
        if (h1 && data.title) {
          h1.innerHTML = `${data.title} - Level <span id="timer-level"></span>`;
        }
        initTimer();
      })
      .catch(err => {
    ...
    // ←ここにタイトル更新を追加する！
    const h1 = document.getElementById("timer-title");
    if (h1 && state.config.title) {
      h1.innerHTML = `${state.config.title} - Level <span id="timer-level"></span>`;
    }
  ...
  function updateButton() {
    ...
    if (state.timerInterval) {
      btn.innerHTML = state.isPaused
        ? `<svg class="icon"><use href="../data/icons-sprite.svg#icon-play" /></svg> Resume`
        : `<svg class="icon"><use href="../data/icons-sprite.svg#icon-pause" /></svg> Pause`;
    } else {
      btn.innerHTML = `<svg class="icon"><use href="../data/icons-sprite.svg#icon-play" /></svg> Start`;
    }
  }
